...
static uint8_t current_address = 0; // Used for tracking the address of the pattern to be displayed
/* USER CODE END PV */
...
int main(void) {
	...
	for (size_t i = 0; i < 6; i++) {
		write_to_address(i, patterns[i]); // Write all the patterns to the EEPROM
	}

	// Flag to check if the button was pressed before
	uint8_t is_half = 0;
	/* USER CODE END 2 */
	...
	while (1) {
		...
		if (!(LL_GPIO_ReadInputPort(Button0_GPIO_Port) & Button0_Pin)){
			htim16.Init.Period = (1000 - 1) * !is_half + (500 - 1) * is_half; 
			is_half = !is_half; // Change the state of the flag
...
void TIM16_IRQHandler(void) {
	...
	uint8_t current_pattern = read_from_address(current_address); // read from address
	LL_GPIO_ResetOutputPin(LED0_GPIO_Port, 0b11111111); // reset all the LEDs

	// Check if the current pattern was written to the EEPROM
	LL_GPIO_SetOutputPin(LED0_GPIO_Port, current_pattern == patterns[current_address] ? current_pattern : 0x01); 
	current_address = (current_address + 1) % 6; // Move on to the next pattern
}
